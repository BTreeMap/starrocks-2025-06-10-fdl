...
TEST_F(DecimalV2ValueTest, string_to_decimal) {
    ...
    ASSERT_EQ(0, value.parse_from_str(".", 1));
    ASSERT_EQ("0", value.to_string());

    ASSERT_EQ(0, value.parse_from_str("100.", 4));
    ASSERT_EQ("100", value.to_string());

    ASSERT_EQ(0, value.parse_from_str(" .1 ", 4));
    ASSERT_EQ("0.1", value.to_string());

    ASSERT_EQ(0, value.parse_from_str("999999999999999999.999999999", 28));
    ASSERT_EQ("999999999999999999.999999999", value.to_string());
}

// NOLINTNEXTLINE
TEST_F(DecimalV2ValueTest, invalid_string_to_decimal) {
    DecimalV2Value value;

    ASSERT_NE(0, value.parse_from_str("", 0));
    // garbage trailing.
    ASSERT_NE(0, value.parse_from_str("1.23a", 5));
    // garbage leading.
    ASSERT_NE(0, value.parse_from_str("a1.23", 5));
}
...
TEST_F(DecimalV2ValueTest, parse_high_scale_decimal_string) {
    ...
    for (auto& tc : test_cases) {
        ...
        auto ok = value.parse_from_str(s.c_str(), s.size());
        ASSERT_EQ(ok, 0);
