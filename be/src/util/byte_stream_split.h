...
#define STARROCKS_HAVE_SIMD_SPLIT
#endif

namespace starrocks::ByteStreamSplitUtil {
...
void ByteStreamSplitDecodeSimd128(const uint8_t* data, int width, int64_t num_values, int64_t stride, uint8_t* out) {
    ...
    for (int64_t i = 0; i < num_blocks; ++i) {
        for (int j = 0; j < kNumStreams; ++j) {
            stage[0][j] = simd_batch::load_unaligned(&data[i * sizeof(simd_batch) + j * stride]);
        }
        for (int step = 0; step < kNumStreamsLog2; ++step) {
            for (int j = 0; j < kNumStreamsHalf; ++j) {
                stage[step + 1U][j * 2] = xsimd::zip_lo(stage[step][j], stage[step][kNumStreamsHalf + j]);
                stage[step + 1U][j * 2 + 1U] = xsimd::zip_hi(stage[step][j], stage[step][kNumStreamsHalf + j]);
            }
        }
        for (int j = 0; j < kNumStreams; ++j) {
            xsimd::store_unaligned(reinterpret_cast<int8_t*>(out + (i * kNumStreams + j) * sizeof(simd_batch)),
...
template <int kNumStreams>
void ByteStreamSplitEncodeSimd128(const uint8_t* raw_values, int width, const int64_t num_values,
    ...
    for (int i = 0; i < kNumStreams; ++i) {
        output_buffer_streams[i] = reinterpret_cast<int8_t*>(&output_buffer_raw[num_values * i]);
    }
    ...
    for (int64_t block_index = 0; block_index < num_blocks; ++block_index) {
        ...
        for (int i = 0; i < kNumStreams; ++i) {
            stage[0][i] = simd_batch::load_unaligned(reinterpret_cast<const int8_t*>(raw_values) +
        ...
        for (int stage_lvl = 0; stage_lvl < 2; ++stage_lvl) {
            for (int i = 0; i < kNumStreams / 2; ++i) {
                stage[stage_lvl + 1][i * 2] = xsimd::zip_lo(stage[stage_lvl][i * 2], stage[stage_lvl][i * 2 + 1]);
                stage[stage_lvl + 1][i * 2 + 1] = xsimd::zip_hi(stage[stage_lvl][i * 2], stage[stage_lvl][i * 2 + 1]);
            }
        }
        if constexpr (kNumStreams == 8) {
            ...
            for (int i = 0; i < 4; ++i) {
                tmp[i * 2] =
                        from_int32_batch(xsimd::zip_lo(to_int32_batch(stage[2][i]), to_int32_batch(stage[2][i + 4])));
                tmp[i * 2 + 1] =
                        from_int32_batch(xsimd::zip_hi(to_int32_batch(stage[2][i]), to_int32_batch(stage[2][i + 4])));
            }
            for (int i = 0; i < 4; ++i) {
                final_result[i * 2] =
                        from_int32_batch(xsimd::zip_lo(to_int32_batch(tmp[i]), to_int32_batch(tmp[i + 4])));
                final_result[i * 2 + 1] =
                        from_int32_batch(xsimd::zip_hi(to_int32_batch(tmp[i]), to_int32_batch(tmp[i + 4])));
            }
        } else {
            ...
            for (int i = 0; i < 2; ++i) {
                final_result[i * 2] =
                        from_int64_batch(xsimd::zip_lo(to_int64_batch(tmp[i]), to_int64_batch(tmp[i + 2])));
                final_result[i * 2 + 1] =
                        from_int64_batch(xsimd::zip_hi(to_int64_batch(tmp[i]), to_int64_batch(tmp[i + 2])));
            }
        }
        for (int i = 0; i < kNumStreams; ++i) {
            xsimd::store_unaligned(&output_buffer_streams[i][block_index * sizeof(simd_batch)], final_result[i]);
        }
    }
}
...
#if defined(__AVX2__)
template <int kNumStreams>
void ByteStreamSplitDecodeAvx2(const uint8_t* data, int width, int64_t num_values, int64_t stride, uint8_t* out) {
    assert(width == kNumStreams);
    static_assert(kNumStreams == 4 || kNumStreams == 8, "Invalid number of streams.");
    constexpr int kNumStreamsLog2 = (kNumStreams == 8 ? 3 : 2);
    ...
    for (int64_t i = 0; i < num_blocks; ++i) {
        for (int j = 0; j < kNumStreams; ++j) {
            stage[0][j] = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(&data[i * sizeof(__m256i) + j * stride]));
        }

        for (int step = 0; step < kNumStreamsLog2; ++step) {
            for (int j = 0; j < kNumStreamsHalf; ++j) {
                stage[step + 1][j * 2] = _mm256_unpacklo_epi8(stage[step][j], stage[step][kNumStreamsHalf + j]);
                stage[step + 1][j * 2 + 1] = _mm256_unpackhi_epi8(stage[step][j], stage[step][kNumStreamsHalf + j]);
            }
        }
        ...
        if constexpr (kNumStreams == 8) {
            ...
            final_result[0] =
                    _mm256_permute2x128_si256(stage[kNumStreamsLog2][0], stage[kNumStreamsLog2][1], 0b00100000);
            final_result[1] =
                    _mm256_permute2x128_si256(stage[kNumStreamsLog2][2], stage[kNumStreamsLog2][3], 0b00100000);
            final_result[2] =
                    _mm256_permute2x128_si256(stage[kNumStreamsLog2][4], stage[kNumStreamsLog2][5], 0b00100000);
            final_result[3] =
                    _mm256_permute2x128_si256(stage[kNumStreamsLog2][6], stage[kNumStreamsLog2][7], 0b00100000);
            final_result[4] =
                    _mm256_permute2x128_si256(stage[kNumStreamsLog2][0], stage[kNumStreamsLog2][1], 0b00110001);
            final_result[5] =
                    _mm256_permute2x128_si256(stage[kNumStreamsLog2][2], stage[kNumStreamsLog2][3], 0b00110001);
            final_result[6] =
                    _mm256_permute2x128_si256(stage[kNumStreamsLog2][4], stage[kNumStreamsLog2][5], 0b00110001);
            final_result[7] =
                    _mm256_permute2x128_si256(stage[kNumStreamsLog2][6], stage[kNumStreamsLog2][7], 0b00110001);
        } else {
        ...
        for (int j = 0; j < kNumStreams; ++j) {
            _mm256_storeu_si256(reinterpret_cast<__m256i*>(out + (i * kNumStreams + j) * sizeof(__m256i)),
                                final_result[j]);
        }
    }
}
...
template <int kNumStreams>
void ByteStreamSplitEncodeAvx2(const uint8_t* raw_values, int width, const int64_t num_values,
                               uint8_t* output_buffer_raw) {
    assert(width == kNumStreams);
    static_assert(kNumStreams == 4 || kNumStreams == 8, "Invalid number of streams.");
    ...
    if constexpr (kNumStreams == 8) // Back to SSE, currently no path for double.
        return ByteStreamSplitEncodeSimd128<kNumStreams>(raw_values, width, num_values, output_buffer_raw);
    ...
    for (int i = 0; i < kNumStreams; ++i) {
        output_buffer_streams[i] = reinterpret_cast<__m256i*>(&output_buffer_raw[num_values * i]);
    }
    ...
    static const __m256i kPermuteMask = _mm256_set_epi32(0x07, 0x03, 0x06, 0x02, 0x05, 0x01, 0x04, 0x00);
...
#if defined(STARROCKS_HAVE_SIMD_SPLIT)
template <int kNumStreams>
void inline ByteStreamSplitDecodeSimd(const uint8_t* data, int width, int64_t num_values, int64_t stride,
                                      uint8_t* out) {
#if defined(__AVX2__)
...
template <int kNumStreams>
void inline ByteStreamSplitEncodeSimd(const uint8_t* raw_values, int width, const int64_t num_values,
                                      uint8_t* output_buffer_raw) {
#if defined(__AVX2__)
    return ByteStreamSplitEncodeAvx2<kNumStreams>(raw_values, width, num_values, output_buffer_raw);
#elif defined(__SSE4_2__) || defined(__ARM_NEON)
    return ByteStreamSplitEncodeSimd128<kNumStreams>(raw_values, width, num_values, output_buffer_raw);
#else
...
inline void DoSplitStreams(const uint8_t* src, int width, int64_t nvalues, uint8_t** dest_streams) {
    ...
    while (nvalues >= kBlockSize) {
        for (int stream = 0; stream < width; ++stream) {
            ...
            for (int i = 0; i < kBlockSize; i += 8) {
                ...
                uint64_t r = a | (b << 8) | (c << 16) | (d << 24) | (e << 32) | (f << 40) | (g << 48) | (h << 56);
#else
...
inline void DoMergeStreams(const uint8_t** src_streams, int width, int64_t nvalues, uint8_t* dest) {
    // Value empirically chosen to provide the best performance on the author's machine
...
template <int kNumStreams>
void ByteStreamSplitEncodeScalar(const uint8_t* raw_values, int width, const int64_t num_values, uint8_t* out) {
    assert(width == kNumStreams);
...
template <int kNumStreams>
void ByteStreamSplitDecodeScalar(const uint8_t* data, int width, int64_t num_values, int64_t stride, uint8_t* out) {
    assert(width == kNumStreams);
...
inline void ByteStreamSplitEncode(const uint8_t* raw_values, int width, const int64_t num_values, uint8_t* out) {
#if defined(STARROCKS_HAVE_SIMD_SPLIT)
...
inline void ByteStreamSplitDecode(const uint8_t* data, int width, int64_t num_values, int64_t stride, uint8_t* out) {
#if defined(STARROCKS_HAVE_SIMD_SPLIT)
...
#undef STARROCKS_HAVE_SIMD_SPLIT
