...
std::unique_ptr<Hasher> HasherFactory::create(Hasher::HashStrategy strategy, uint32_t seed) {
    if (strategy == Hasher::HashStrategy::HASH_MURMUR3_X64_64) {
        return std::make_unique<MurmurHasher>(seed);
    } else if (strategy == Hasher::HashStrategy::XXHASH64) {
        return std::make_unique<XXHasher>(seed);
    } else {
        return nullptr;
    }
}
